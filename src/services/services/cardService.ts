...
export default class CardService {
	static async updateCardBalance(): Promise<any> {
		return updateDateStateWithSession(api.card.balance, null, 'setCardBalance', (res: FetchResult) => res.data.data.balance);
	}

	static async updateCardHistory(data?: { year: number; month: number }): Promise<any> {
		return updateDateStateWithSession(api.card.history, data, 'setCardHistory', (res: FetchResult) => {
			return {
				data: res.data.data,
				year: data?.year,
				month: data?.month
