<template>
   <quick-view title="考试安排" icon-name="lessonstable" @tap="nav3Exam" help @handle-tap-help="handleTapHelp" class="exam-quick-view">
       <text class="sub-text">近期考试 ({{updateTimeString}})</text>
     <card v-if="!filteredExamItems || filteredExamItems.length === 0" style="text-align: center">未查询到近日考试信息</card>
          ...
          <view>
            ...
            <view>
               ...
               <view class="exam-res">
                 ...
                 <text v-if="examState(item.examTime) === 'before'">还有 {{getRestTimeString(item.examTime)}} 开始</text>
               </view>
             </view>
          </view>
       </card>
...
const filteredExamItems = computed(() => {
     ...
     try{
         ...
         const resTime = new Date().getTime() - new Date(examTime[0] + " " + examTime[1] + ":00").getTime();
