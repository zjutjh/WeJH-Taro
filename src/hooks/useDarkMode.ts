...
import { Config, defaultConfig_dark, defaultConfig_light } from "@/store/service/theme";
...
const useDarkMode = () => {
  ...
  const setMode = async (value: DarkModeTheme) => {
    ...
    if(config === undefined) config = value === 'light' ? defaultConfig_light : defaultConfig_dark
