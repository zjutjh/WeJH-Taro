    ...
    <view class="flex-column">
      <card v-if="userInfo?.isActive" class="profile-card">
        <view class="avatar-wrapper">
          <image v-if="userInfo?.wxProfile" class="avatar" :src="userInfo?.wxProfile.avatarUrl" />
        </view>
      ...
      <card v-else title="未激活，激活享受更多精彩">
        <w-button v-if="!userInfo?.isActive" block class="active" @tap="nav2activation">
          激活
        </w-button>
      </card>

      <view v-if="userInfo?.isActive" class="operate">
        <w-list v-for="(column, index) in options" :key="index" class="operate-list">
...
import { Card, TitleBar, WBadge, WButton, WList, WListItem } from "@/components";
...
onMounted(() => {
  if (serviceStore.user.isActive && !serviceStore.user.info) UserService.getUserInfo();
});
