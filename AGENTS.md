...
- 页面目录下的 `AGENTS.md` 既是就近生效的 Agent 说明，也是该页面人工测试用例的唯一事实来源。文件开头必须明确写明“本文件是本页面的测试用例文档”，避免与根目录项目说明混淆。
- 用例使用稳定 ID 和 `P0` / `P1` / `P2` 优先级；修改行为时同步检索并更新关联用例，不按章节顺序重新编号。
- 每条用例必须包含 `优先级`、`执行方式`、`前置条件`、`操作步骤`、`预期结果`、`关联代码`。`预期结果` 必须记录当前实现可重复观察到的行为，不以理想目标替代实际；当前行为仍有缺陷或改进目标时，另写 `已知风险` 说明影响，并在实现修复后同步更新预期。
- `关联代码` 使用“相对仓库路径 → 符号名/模板条件”的格式，不写易随代码变动失效的行号。
- 跨页面场景按数据或业务逻辑的所有者归档：所有者页面保存完整用例，消费页面只链接到该用例，避免复制后分别过期。
...
- 操作步骤：<可重复执行的操作>
- 预期结果：<明确的通过条件>
- 关联代码：`<path>` → `<symbol/template condition>`
- 已知风险：<当前行为的缺陷、影响与改进目标，按需填写>
```
